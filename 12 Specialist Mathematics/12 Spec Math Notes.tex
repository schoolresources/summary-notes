...
\section{Vectors}
	...
	The angle $\theta$ between two vectors $\vb{a}$ and $\vb{b}$ cacn be found using
	\begin{align}
	...
	\begin{itemize}
		...
		\item $\vb{a}\times\vb{b}=-\vb{b}\times\vb{a}$, $\therefore\vb{a}\times\vb{b}$ and $\vb{b}\times\vb{a}$ hav equal length but opposite direction.
		\item $\vb{a}\cdot(\vb{b}\times\vb{c})$ is called the scalar triple prodcut.
	...
	\begin{itemize}
		\item Lines are coplanar if the lie in the same plane. In this case they may be intersecting, parallel or coincident.
	...
	\begin{itemize}
		...
		\item $\vb{v}$ and $\vb{w}$ are any two non-parallel vectors which are parallel to the plane.
	...
	and passes through $P(X,Y,Z)$ then is has equation
	\begin{align}
		ax+by+cz=aX=bY=cZ=d
	\end{align}
	...
	\begin{align}
		d=\frac{|Ax_1+Bx_1+Cx_1|}{\sqrt{A^2+B^2+C^2}}
	\end{align}
